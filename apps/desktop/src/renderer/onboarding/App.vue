...
const STEP_LABELS = computed(() => [
  ...
  t('onboarding.stepModel'),
  t('onboarding.stepCharacter'),
  t('onboarding.stepInteract'),
]);
...
async function finish(): Promise<void> {
  await window.openpet.rpc('app.window.finishOnboarding', {});
}
...
<template>
  <div class="flex h-screen flex-col p-6 text-base" style="background: var(--ds-glass-bg)">
    ...
    <ConfirmDialog
      ...
      :detail="t('onboarding.skipConfirmDetail')"
      :confirm-label="t('onboarding.skipConfirmLabel')"
      @confirm="confirmSkip"
