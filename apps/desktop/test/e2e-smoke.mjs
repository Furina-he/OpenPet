...
process.env.OPENPET_DEFAULT_PROVIDER = 'mock';
...
async function main() {
  ...
  await Promise.all(
    [character, overlay].map((w) =>
      waitFor(
        () =>
          w.webContents
            .executeJavaScript('typeof window.openpet')
            .then((t) => t === 'object'),
        'preload bridge',
      ),
    ),
  );
  console.log('[smoke] window.openpet bridge present in both renderers');

  // character 侧安插行为通道探针
  await character.webContents.executeJavaScript(`
    window.__smoke = { emotions: [], actions: 0, intents: 0 };
    window.openpet.on('behavior.applyEmotion', (p) => window.__smoke.emotions.push(p.name));
    window.openpet.on('behavior.playAction', () => window.__smoke.actions++);
    window.openpet.on('behavior.setIntent', () => window.__smoke.intents++);
    'ok';
  `);

  // overlay 侧安插文本流探针 + sys.ping 健康检查
  const ping = await overlay.webContents.executeJavaScript(
    `window.openpet.rpc('sys.ping', { nonce: 'smoke' })`,
  );
  ...
  const rejected = await overlay.webContents.executeJavaScript(
    `window.openpet.rpc('chat.send', { wrong: 1 }).then(() => false, (e) => String(e.message))`,
  );
  ...
  await overlay.webContents.executeJavaScript(`
    ...
    window.openpet.on('chat.stream', (p) => { if (p.sessionId === 'smoke-s1') window.__smoke.text += p.text; });
    window.openpet.on('chat.done', (p) => { if (p.sessionId === 'smoke-s1') window.__smoke.done = p.finishReason; });
    window.openpet.rpc('chat.send', { sessionId: 'smoke-s1', text: 'hello' });
  `);
  ...
  `);
  await overlay.webContents.executeJavaScript(
    `window.openpet.rpc('chat.send', { sessionId: 'smoke-s2', text: 'again' })`,
  );
  ...
  await overlay.webContents.executeJavaScript(`
    ...
    window.openpet.rpc('chat.send', { sessionId: 'smoke-cancel', text: 'cancel me' });
    'ok';
  `);
  await waitFor(
  ...
  await overlay.webContents.executeJavaScript(
    `window.openpet.rpc('chat.cancel', { sessionId: 'smoke-cancel' })`,
  );
  ...
  await overlay.webContents.executeJavaScript(`
    ...
    window.openpet.on('chat.done', (p) => { if (p.sessionId === 'default') window.__snapDone = p.finishReason; });
    window.openpet.rpc('chat.send', { sessionId: 'default', text: '快照测试' });
    'ok';
  `);
  await waitFor(
  ...
  const snap = await overlay.webContents.executeJavaScript(
    `window.openpet.rpc('chat.snapshot', { sessionId: 'default' })`,
  );
  ...
  await character.webContents.executeJavaScript(`(() => {
    ...
    window.openpet.on('behavior.playAction', (p) => { window.__m4ActionSeen = p; });
    return window.openpet.rpc('character.idleTimeout', { idleMs: 90000 });
  })()`);
  ...
  for (let i = 0; i < 80; i++) {
    await character.webContents.executeJavaScript(
      `window.openpet.rpc('app.window.moveBy', { dx: 2, dy: 1 })`,
    );
  }
