...
// 运行：pnpm build && pnpm --filter @openpet/desktop exec electron test/m4-fps-probe.mjs
...
async function main() {
  ...
  character.webContents.send('openpet:notify:behavior.applyEmotion', { name: 'happy', weight: 1 });
  await sleep(700);
  await capture(character, 'scale100-happy.png');
  character.webContents.send('openpet:notify:behavior.applyEmotion', { name: 'sleepy', weight: 1 });
  await sleep(700);
  await capture(character, 'scale100-sleepy.png');
  character.webContents.send('openpet:notify:behavior.applyEmotion', { name: 'neutral', weight: 0 });

  // 动作通道驱动 wave，动作中段截图（人工核对程序化动作形态）
  character.webContents.send('openpet:notify:behavior.playAction', {
    name: 'wave',
    durationMs: 1800,
  });
  await sleep(900);
  ...
  for (const scale of [0.5, 1, 2]) {
    await overlay.webContents.executeJavaScript(
      `window.openpet.rpc('character.setScale', { scale: ${scale} })`,
    );
