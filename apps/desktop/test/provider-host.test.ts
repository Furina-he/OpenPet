...
describe('ProviderHost · supervision (S2 semantics)', () => {
  // 批次⑥ T9：计时断言全部改注入 delay（零等待）+ 轮询条件——断言退避「序列数值」，
  // 不再赌真实墙钟（并行满载下 worker spawn 慢导致的偶发红）。
  it('synthesizes an error done when the worker dies mid-stream, then recovers', async () => {
    ...
    // 重生已调度（delay 立即 resolve → spawn 同步建新 worker），新流可用
    await until(() => respawns >= 1, 'respawn scheduled');
    host.send('s-after');
  ...
  it('escalates backoff exponentially while the worker keeps crashing', async () => {
    ...
    await until(() => waits.length >= 3, 'three crash-respawn rounds');
  ...
  it('caps backoff at maxBackoffMs', async () => {
    ...
    await until(() => waits.length >= 4, 'four crash-respawn rounds');
  ...
  it('resets backoff only after a healthy response (proof of life)', async () => {
    ...
    host.killWorkerForTest();
    await until(() => waits.length === 1, 'first respawn');
    // 再来一轮：健康 → 杀 → 等待仍应是 base（配合 escalates 用例共同钉死「只有响应才重置」）
    host.send('h2');
    await untilDone(events, 'h2');
    host.killWorkerForTest();
    await until(() => waits.length === 2, 'second respawn');
