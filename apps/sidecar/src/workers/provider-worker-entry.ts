...
import { installFetchProxy } from '@openpet/plugin-sdk';
...
import { embed } from './providers/embedding.js';

// 兼容别名：帧定义已收口到 @openpet/protocol（单一真源）。
...
export function attachProviderServer(port: MessagePort): void {
  ...
  port.on('message', (msg: InboundMessage) => {
    ...
    if (msg.kind === 'embed.request') {
      // §5 KB / 自动 RAG：format 由 adapter 映射（非 ollama 一律 openai 兼容端点）。
      const format = msg.adapter === 'ollama' ? 'ollama' : 'openai';
      embed({ inputs: msg.inputs, model: msg.model, baseUrl: msg.baseUrl ?? '', format })
        .then((vectors) =>
          port.postMessage({ kind: 'embed.result', requestId: msg.requestId, vectors }),
        )
        .catch((e: unknown) =>
          port.postMessage({
            kind: 'embed.error',
