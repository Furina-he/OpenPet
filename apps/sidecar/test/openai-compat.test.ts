...
import { openaiCompatChat, toWireMessage } from '../src/workers/providers/openai-compat.js';
...
describe('openaiCompatChat', () => {
  ...
  it('maps reasoning_content delta to a reasoning event; content stays a delta (C′)', async () => {
    globalThis.fetch = vi.fn(async () =>
      sseResponse([
    ...
    const ev = await collect(openaiCompatChat(dialect, req, new AbortController().signal));
    expect(ev).toContainEqual({ type: 'reasoning', text: '想' });
    expect(ev).toContainEqual({ type: 'delta', text: '答' });
  });
});

describe('toWireMessage（§5 工具回灌 wire 映射）', () => {
  it('assistant+toolCalls → tool_calls 数组 + content null（照 3db778f 规范）', () => {
    expect(
      toWireMessage({
        role: 'assistant',
        content: '',
        toolCalls: [{ id: 'c1', name: 'get_time', argsJson: '{"tz":"utc"}' }],
      }),
    ).toEqual({
      role: 'assistant',
      content: null,
  ...
  it('tool → tool_call_id', () => {
    expect(toWireMessage({ role: 'tool', content: '12:00', toolCallId: 'c1' })).toEqual({
      role: 'tool',
      content: '12:00',
      tool_call_id: 'c1',
    });
