...
Renderer 经 `window.openpet.rpc(...)` 调到 Main;Main 内 JSON-RPC 路由把请求转发给 `worker_threads` 内的 sidecar 业务模块;Worker 崩溃后由 PluginHost 指数退避重启,重连后可继续调用。语义模型全程 JSON-RPC 2.0,`@openpet/protocol` 零改动复用。
...
`test/plugin-host.test.ts`(Vitest,跨平台,6 个用例全过)。用真实 sidecar worker 入口(`@openpet/sidecar/dist/worker-entry.js`,即生产 Main resolve 的同一文件)跑往返/重连;用 `test/fixtures/crash-worker.mjs`(启动即抛)验退避递增——因为永远收不到健康响应,退避不会被重置,才能观察到 1→2→4 倍增。
...
**`@openpet/protocol` 的 `dist/index.js` 相对导出缺 `.js` 后缀。**
...
修复:`src/index.ts` 三个 `export *` 补 `.js` 后缀(符合仓库约定"ESM 相对导入要带 `.js` 后缀")。已重建 protocol dist 验证。
...
| 检查项 | 通过? | 备注 |
| --- | --- | --- |
| dev 窗口弹出,ping 按钮返回 `{pong:'ok', echoNonce}` | ☐ | `pnpm --filter @openpet/spike-s2 dev` |
