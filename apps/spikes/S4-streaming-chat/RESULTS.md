...
| 检查项 | 结果 | 说明 |
| --- | --- | --- |
| `@desksoul/protocol` 新 method schema 单测 | ✅ | 8 个用例（含 chat.* / behavior.* params/result 往返 + 缺字段拒绝） |
| `@desksoul/sidecar` mock-provider 流式 + 取消单测 | ✅ | 3 用例：全量 stop / 中途 abort→cancel / 起始即 abort |
| `@desksoul/sidecar` provider-worker-entry MessagePort 往返 | ✅ | 2 用例：真实 MessageChannel 驱动 start→event 流 + cancel |
...
| `tsc` 类型检查（node + renderer 两份 tsconfig） | ✅ | `pnpm --filter @desksoul/spike-s4 typecheck` |
...
- 新增的 `chat.*` / `behavior.*` method 已进 `@desksoul/protocol`，迁移到 `apps/desktop` 时直接复用，勿重复定义。
