 ...
 * ConversationCore is fanned to BOTH windows on `openpet:notify:<channel>`.
...
export function registerRpc(deps: RouterDeps): { dispose: () => Promise<void> } {
  const broadcast = (n: Notification): void => {
    for (const wc of deps.targets()) {
      if (!wc.isDestroyed()) wc.send(`openpet:notify:${n.channel}`, n.params);
    }
  };
  ...
  ipcMain.handle('openpet:rpc', (_e, { method, params }: { method: string; params?: unknown }) => {
    switch (method) {
      case 'chat.send': {
