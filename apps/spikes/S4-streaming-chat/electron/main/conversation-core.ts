/**
 * ConversationCore — the dual-channel splitter (Main-side, S4 scope).
 *
 ...
 */
import { BehaviorParser, type BehaviorEvent } from '@desksoul/protocol';
import type { ChatEvent } from './provider-host.js';
  ...
  private emitBehavior(sessionId: string, be: BehaviorEvent): void {
    switch (be.type) {
      ...
      case 'intent':
        ...
        break;
      case 'wait':
        // S4 does not pace the text stream on <wait/>; ignore (handled in MVP M3).
        break;
    }
  }
}
