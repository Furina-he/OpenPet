...
sendBtn.addEventListener('click', async () => {
  ...
  setStreaming(true);
  try {
    await window.openpet.rpc('chat.send', { sessionId: SESSION_ID, text: 'hello' });
  } catch (e) {
    setStreaming(false);
...
cancelBtn.addEventListener('click', () => {
  if (!streaming) return;
  void window.openpet.rpc('chat.cancel', { sessionId: SESSION_ID });
