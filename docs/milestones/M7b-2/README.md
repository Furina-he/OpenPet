...
| 阶段 | plan | 内容 | 状态 |
| --- | --- | --- | --- |
| P1 | plans/p1-foundation | `onboarding.completed` prefs + `finishOnboarding` RPC + 首启判定 + 引导窗 + electron-vite entry + 空壳 | ⏳ 待开 |
| P2 | plans/p2-shell-c1-c4 | wizard 状态机 + 步骤指示器/跳过确认 + C1 欢迎 + C4 首句（chip→overlay）+ 完成页 | ⏳ |
| P3 | plans/p3-c2-model | 抽 `ProviderConfigPanel`（D3 复用不回归）+ C2 LLM 配置（两路径 + 隐私条 + 跳过演示） | ⏳ |
| P4 | plans/p4-c3-demo-acceptance | C3 角色选择（默认角色 + 浏览禁用）+ demo 台词池 + 视觉保真 pass + RESULTS | ⏳ |
