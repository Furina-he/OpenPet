...
DeskSoul 是桌面常驻的 AI 角色伙伴（桌宠 + AI 内核 + 角色/插件生态）。当前处于 **Phase 2（MVP 切片）** 阶段：M1（架构骨架 + spike 迁移）、M2（IPC 四命名空间 + 取消 + 背压 + chat.snapshot 恢复）、M3（行为协议生产化：全标签集 + fail-safe + wait 节流 + 覆盖率门槛）、M4（渲染层 CharacterRuntime：全接口 + asset:// 资产安全加载 + LookAt/Idle 池/90s 主动行为/缩放/性能预算）、M5（Provider 插件运行时：单 worker 多 provider 注册表 + 流式 fetch 网关 Main 侧密钥注入 + openai-compat/Ollama/Claude/Gemini + 降级链 + tool_call 单轮回灌 + Embedding + token 统计与估算兜底 + provider.* RPC）已完成，下一个里程碑是 M6（状态层 Working/Persona State + 数据层 SQLite）。完整产品需求见 `PRD.md`，架构权威来源是 `docs/plans/2026-05-01-desksoul-tech-design.md`（v0.2 Electron Pivot），任务清单见 `docs/plans/2026-05-01-desksoul-impl-plan.md`。
