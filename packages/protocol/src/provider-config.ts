...
export const ProviderSourceSchema = z.object({
  ...
  /** 显示名 + 厂商/图标键（建源时从具名模板带入；旧源无→列表退回显示 id/adapter）。 */
  name: z.string().optional(),
  icon: z.string().optional(),
  /** 类型专属配置（照 AstrBot 各 provider_type 字段：embedding dimensions / tts voice /
   *  rerank suffix / agent appId 等）；按 providerConfigMeta(capability) 渲染。 */
...
/**
 * embeddingModelId → ModelEntry → ProviderSource；缺失/disabled 返回 null。
 * 与 chat 解析同逻辑（embedding 与 chat 共用两层 source+model 配置），名表意。
 */
export function resolveEmbeddingTarget(
  ...
  return resolveChatTarget(sources, models, embeddingModelId);
}

/** rerank 目标：model.defaultRerankModelId → 两层解析（同 chat/embedding）。 */
export function resolveRerankTarget(
  sources: ProviderSource[],
  models: ModelEntry[],
