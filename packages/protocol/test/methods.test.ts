...
describe('app.window.* methods', () => {
  ...
  it('registers hideSelf with empty params (overlay × 收起)', () => {
    expect(Methods['app.window.hideSelf'].params.safeParse({}).success).toBe(true);
    expect(Methods['app.window.hideSelf'].result.safeParse({ ok: true }).success).toBe(true);
  });
});
